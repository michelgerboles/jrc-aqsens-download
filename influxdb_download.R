influxConfig <- function(boxName) {
  ...
  return(CONFIG(DisqueFieldtestDir = boxDirectory,
...
influxDownload <- function(boxName, boxConfig) {
  ...
  if (file.exists(InfluxData.file)) {
    if (extension(InfluxData.file) == ".csv") {
      ...
      if (!"" %in% boxConfig$Server$Influx.TZ) {
  ...
  if (file.exists(SOSData.file)) {
    if (extension(SOSData.file) == ".csv") {
      ...
      if (!"" %in% boxConfig$Server$SOS.TZ) {
        data.table::set(Sos, j = "date", value =  ymd_hms(Sos[["date"]], tz = boxConfig$Server$SOS.TZ))
  ...
  if (file.exists(RefData.file)) {
    if (extension(RefData.file) == ".csv") {
      ...
      if (!"" %in% boxConfig$Server$ref.tzone) {
  ...
  i.sensors          <- which(!is.na(Set.Time$name.gas) & Set.Time$name.gas %in% boxConfig$sens2ref$name.gas)
  ...
  list.gas.reference2use <- boxConfig[["sens2ref"]]$gas.reference2use[!is.na(boxConfig[["sens2ref"]]$gas.reference2use) &
                                                                     boxConfig[["sens2ref"]]$gas.reference2use != "" &
                                                                     boxConfig[["sens2ref"]]$gas.sensor %in% list.gas.sensor]
  # return a vector with the names of gas reference using the file ASE.cfg
  list.gas.reference <- boxConfig[["sens2ref"]]$gas.reference[!is.na(boxConfig[["sens2ref"]]$gas.reference) &
                                                             boxConfig[["sens2ref"]]$gas.reference != "" &
                                                             boxConfig[["sens2ref"]]$gas.sensor %in% list.gas.sensor]
  ...
  if (any(grepl(pattern = ",", x = boxConfig$Server$urlref))) urlref = unlist(strsplit(gsub(pattern = " ","",x = boxConfig$Server$urlref), split = ",")  ) else urlref = gsub(pattern = " ","",x = boxConfig$Server$urlref)
  # Setting REFDATA
  ...
  if (Download.Sensor$ExistFil.data.Ref) {
    ...
    if (Download.Sensor$ExistFil.data.Influx) {
      ...
      Ref.start <- as.Date(format(Sys.time(), tz = boxConfig$Server$ref.tzone))
    }
  }
  ...
  #                urlref             = urlref,
  #                sens2ref           = boxConfig$all[["sens2ref"]],
  ...
  #                RefSOSDateEND      = as.Date(format(Sys.time(), tz = boxConfig$Server$ref.tzone)) + 1,
  ...
  #                Ref__a_i_p__Station      = boxConfig$Server$Ref__a_i_p__Station,
  ...
  #                Ref__a_i_p__DateEND      = as.Date(format(Sys.time(), tz = boxConfig$Server$ref.tzone)) + 1,
  #                csvFile            = boxConfig$Server$file1,
  #                csvFile.sep        = boxConfig$Server$sep,
  #                csvFile.quote      = boxConfig$Server$quote,
  #                coord.ref          = boxConfig$Server$coord.ref,
  #                Ref.Type           = boxConfig$Server$Ref.Type,
  #                RefData            = RefData,
  #                shiny              = FALSE)
  ...
  if (!is.null(Ref) && Outliers.Ref.Forced) {
    ...
    for (i in list.gas.reference2use) {
      ...
      if (boxConfig$sens2ref$Ref.rm.Out[k]) {
      ...
      set(DT.General, j = paste0("Out.",i,".",1:boxConfig$sens2ref$Ref.iterations[k]),
          value = rep(list(DT.General[[paste0("Out.Neg.",i)]]), times = boxConfig$sens2ref$Ref.iterations[k]))
      # deleting bigger iterations
      j  <- boxConfig$sens2ref$Ref.iterations[k]
      repeat (
        if (any(grepl(pattern = paste0("Out.",i,".", j + 1)      , x = names(DT.General)))) {
          set(DT.General, j = paste0("Out.",i,".",j + 1), value = NULL)
          j <- j + 1
      ...
      if (boxConfig$sens2ref$Ref.rm.Out[k]) {
        for (j in 1:boxConfig$sens2ref$Ref.iterations[k]) { # numver of iterations
          if (i %in% names(DT.General)) {
            if (all(is.na(DT.General[[i]]))) {
            } else {
              ...
              Outli <- My.rm.Outliers(ymin         = boxConfig$sens2ref$Ref.Ymin[k],
                                      ymax         = boxConfig$sens2ref$Ref.Ymax[k],
                                      ThresholdMin = boxConfig$sens2ref$Ref.ThresholdMin[k],
                                      date         = DT.General[["date"]],
                                      y            = Y,
                                      window       = boxConfig$sens2ref$Ref.window[k],
                                      threshold    = boxConfig$sens2ref$Ref.threshold[k],
                                      plotting     = FALSE
              )
              nameInd      <- paste0(i,".",j)
              OutlinameInd <- paste0(i,".",j,".Outli")
    ...
    rm(Outli)
    for (i in 1:length(list.gas.reference2use)) for (j in 1:boxConfig$sens2ref$Ref.iterations[i]) assign(paste0(list.gas.reference2use[i],".",j),NULL)
  }
  ...
  Calib_data <- data.frame(
    ...
    Sens.unit          = as.character(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Sens.unit[i])),
    Cal.Line           = as.character(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Cal.Line[i])),
    Sync.Cal           = as.logical(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Sync.Cal[i])),
    Sync.Pred          = as.logical(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Sync.Pred[i])),
    Cal.func           = as.character(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Cal[i])),
    mod.eta.model.type = as.character(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Calibration[i])),
    Neg.mod            = as.logical(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Neg.mod[i])),
    Slope              = as.numeric(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Slope[i])),
    Intercept          = as.numeric(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Intercept[i])),
    ubsRM              = as.numeric(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$ubsRM[i])),
    ubss               = as.numeric(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$ubss[i])),
    eta.model.type     = as.character(sapply(seq_along(list.gas.sensor), function(i) boxConfig$sens2ref$Comparison[i])),
    stringsAsFactors = FALSE)
